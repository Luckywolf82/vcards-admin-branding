...
// Laster opp logo til GitHub: <basePath>/assets/<orgKey>/<filename>
// Returnerer både Github Pages-URL og RAW-fallback (med cache-bust).
...
exports.handler = async (event) => {
  try {
    ...
    const bust = Date.now();
    // Github Pages URL (kan trenge ~1 min før filen er tilgjengelig)
    const pagesUrl = `https://${owner}.github.io/${repo}/${safeBase}/assets/${safeOrg}/${encodeURIComponent(safeFile)}?v=${bust}`;
    // RAW fallback — tilgjengelig umiddelbart
    const rawUrl   = `https://raw.githubusercontent.com/${owner}/${repo}/${branch}/${encodeURIComponent(path)}?v=${bust}`;
