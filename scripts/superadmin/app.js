...
// Del 6 – del 5/6: Brukere script
  (function(){
    ...
    function clearOrgForm(){
      ...
      if (el.orgLimit) el.orgLimit.value = '';
    ...
    function populateOrgForm(record){
      ...
      if (el.orgLimit) {
        el.orgLimit.value = typeof record.employeeLimit === 'number'
          ? String(record.employeeLimit)
          : (record.employeeLimit || '');
      }
    ...
    function gatherOrgPayload(){
      ...
      const limitRaw = (el.orgLimit?.value ?? '').trim();
      if (limitRaw && !/^\d+$/.test(limitRaw)) {
        setOrgMsg('Maks ansatte/kort må være et heltall ≥ 0.', { type: 'err' });
        return null;
      }
      if (!name) name = key;
      const payload = { key, name, site, accent, logo, logoBg };
      if (limitRaw) {
        payload.employeeLimit = Number(limitRaw);
      } else {
        payload.employeeLimit = '';
      }
      return payload;
    }
    ...
    function renderOrgTable(){
      ...
      const rows = orgState.list.map((org) => {
        ...
        const limitTd = makeCell();
        if (typeof org.employeeLimit === 'number') {
          limitTd.textContent = String(org.employeeLimit);
        } else if (org.employeeLimit) {
          limitTd.textContent = org.employeeLimit;
        } else {
          limitTd.textContent = '—';
        }
      ...
      const promise = request(ORG_API.list)
        .then((data) => {
          ...
          const mapped = raw.map((item) => {
            ...
            const limitValue = item.employeeLimit;
            let employeeLimit = null;
            if (typeof limitValue === 'number' && Number.isFinite(limitValue) && limitValue >= 0) {
              employeeLimit = Math.floor(limitValue);
            } else if (typeof limitValue === 'string' && limitValue.trim()) {
              const num = Number(limitValue.trim());
              if (Number.isFinite(num) && num >= 0) employeeLimit = Math.floor(num);
            }
            return { key, name: name || key, site, accent, logo, logoBg, employeeLimit };
          }).filter(Boolean);
